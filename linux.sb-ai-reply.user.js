...
(function () {
  ...
  // 「不要提问」约束：原措辞明确允许「提出疑问」，导致回帖常以反问/提问收尾
  // （用户反馈：水贴不该向人提问——既显得像不懂，也容易招来互动）。
  // 有疑问/质疑改用陈述句。写成「不要出现问句或问号」对小模型最好执行、也最好验证。
  // 注意：结尾不带句号——它插在「…或给出建议」与「；信息量适中…」之间，带了会出现「）。；」
  const NO_ASK_CLAUSE = '；**回帖里不要出现问句或问号**——不要向任何人提问、不要反问、不要以提问收尾；有疑问或想质疑就用陈述句说出来（例如把「这里 X 和 Y 是不是冲突？」写成「这里 X 和 Y 对不上」）';
  ...
  const DEFAULT_SYSTEM_PROMPT = [
    ...
    '2. **内容要求**：回帖必须与帖子内容紧密相关，体现出对帖子的理解；可以表达赞同、补充细节、分享相关经验或给出建议' + NO_ASK_CLAUSE + '；信息量适中，不要为了凑字而重复；不要复述帖子原文。',
  ...
  const DEFAULT_REPLY_SYSTEM_PROMPT = [
    ...
    '',
    '1. **语言与风格**：使用自然、口语化的中文，避免书面化或翻译腔；不要使用“作为 AI”“我是一个人工智能”等暴露身份的表述；语气友好、真诚，适当使用论坛常用但不过度的网络用语（如“确实”“学习了”“感谢分享”等）。',
    '2. **内容要求**：回应必须紧扣目标评论的观点，体现出你认真看了这条评论；可以赞同、反驳、补充细节或分享相关经验' + NO_ASK_CLAUSE + '；要针对对方的具体说法展开，不要泛泛而谈，也不要跑题到整篇帖子。',
  ...
  const CSS = `
    ...
    /* 搜索结果来源徽标（按来源着色：GN/BN/Brave/Bing/DDG/SO/GH/HN/SearX）
       BN=Bing News(青绿，与 Bing 网页版蓝区分开)；Brave=玫红 */
    ...
    .lsb-src-bn { background: #d1fae5; color: #047857; }
    .lsb-src-brave { background: #ffe4e6; color: #be123c; }
  ...
  // 已存预设的「不要提问」升级：提示词是持久化的（loadPrompts 只在存储为空时播种默认值），
  // 所以只改 DEFAULT_SYSTEM_PROMPT / DEFAULT_REPLY_SYSTEM_PROMPT 这两个常量，对已经用过脚本的人
  // 是不生效的——必须在这里补一次**定向替换**：只替换那两句明确写着「提出疑问」的原话，
  // 其它内容（尤其是用户自己改过的部分）一律不碰。替换幂等，跑第二遍不会有变化。
  function upgradeAskClause(s) {
    let t = String(s || '');
    t = t.replace(
      '可以表达赞同、补充细节、提出疑问、分享相关经验或给出建议',
      '可以表达赞同、补充细节、分享相关经验或给出建议' + NO_ASK_CLAUSE
    );
    t = t.replace(
      '可以赞同、反驳、补充细节、提出疑问或分享相关经验',
      '可以赞同、反驳、补充细节或分享相关经验' + NO_ASK_CLAUSE
    );
    return t;
  }
  ...
  function loadPrompts() {
    ...
    v.forEach((p) => {
      if (!p) return;
      ['systemPrompt', 'replySystemPrompt'].forEach((k) => {
        const nv = upgradeAskClause(p[k]);
        if (nv !== p[k]) { p[k] = nv; upgraded = true; }
      });
    });
    if (upgraded) {
      savePrompts(v);
      // 用 console 而非视奸窗：这里跑在初始化阶段，日志 UI 可能还没建好
      console.log('[水贴专用] 已把本地已存提示词里的「可以提出疑问」升级为「不要提问」（只改了那一句，你的其它改动保留）');
    }
    return v;
  }
  ...
  function buildReplyUserContent(scrapedText, hasMention, target) {
    ...
    return '以下是论坛帖子的主题、正文，以及一条我准备回应的评论（每条已用【发言人】标识区分）。' + who + '，请针对 TA 的那条评论，写一条自然、口语化的中文回帖，可以赞同、补充或给建议，观点要紧扣该评论；回帖里不要出现问句或问号（有疑问就写成陈述句）。请直接输出回帖正文，不要带 @ 前缀或任何解释。\n\n内容：\n' + scrapedText;
  }
  ...
  let ddgFailStreak = 0;
  // Brave 会话级熔断 + 串行队列：Brave 对**并发**请求直接 429（实测 3 并发 × 3 轮 = 9/9 全 429，
  // 而串行逐个请求可稳定成功），故同一时刻只允许一个在途请求，并在连续失败达阈值后本轮跳过。
  ...
  function pickGlobal(cfg, candRows, nWords) {
    ...
    const usr = '本次共 ' + nWords + ' 个搜索词、' + candRows.length + ' 条候选结果。\n'
      + '将对选中的条目抓取网页正文（单条约 500-4000 字）。抓取条数由你按信息价值自行判断（0-10 条，不必凑数）：'
      + '候选低质、或仅凭标题与摘要已够用时少抓或不抓；存在多个真正值得精读全文的强信息源时多抓。宁精勿滥，排除站内首页/登录页/商城/纯目录页/与主题无关的条目。\n'
  ...
  function clientWebSearch(cfg, query) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (engine.smart) {
        ...
        const fetchBrave = (q) => {
          const run = braveQueue.then(() => {
            if (braveFailStreak >= 2) return { ok: false, err: '已熔断(本轮连续' + braveFailStreak + '次失败)', tag: 'Brave' };
            return fetchItems(braveE.buildUrl(q), htmlParse(braveE), 'Brave').then((r) => {
              if (r.ok) braveFailStreak = 0; else braveFailStreak += 1;
              return r;
            });
          });
          braveQueue = run.then(() => {}, () => {});
        ...
        (async () => {
          try {
            ...
            if (isCJK) {
              // 中文链路（按实测「质量 × 可用性」定序）：
              //   · Google News RSS —— 中文资讯最强、实测 0.86~5.8s；通用词也给高质量文章
  ...
  async function agentSearchReply(cfg, rawText, finalUserContent, images, onProgress, hooks) {
    ...
    const clippedSearch = rawSearchBlock.length > SEARCH_SUMMARY_LIMIT
      ...
      : rawSearchBlock;
    // 帖子内容与搜索资料的「来源边界」——实战里最容易出错的地方：模型会把搜来的细节
    // （某方案的应用场景、价格、参数、案例…）当成帖子写过的内容，进而写出「你说的这个我也很赞」
    // 这类张冠李戴的附和（帖子只说了方案 A，搜到 A 的使用场景，回帖却把使用场景当成帖里提过的）。
    // 所以在搜索块之前把边界讲死：先用一行显式声明「帖子内容到此结束」，再给出归属规则。
    const SOURCE_BOUNDARY =
      '\n\n=== 以上为帖子内容（结束）===\n' +
      '【❗来源边界·务必遵守】下面是联网搜到的第三方资料。它与上面的帖子是两份独立内容，写回帖/评议理由时绝不能混：\n' +
      '1. 搜索资料只用于补充背景、核对事实，**它不是帖子内容**，不要把里面的信息当成帖子写过的。\n' +
      '2. 帖子里没提过的细节（某方案的应用场景、价格、参数、案例、评价等），即使搜索资料里有，也不能写成「帖里说的/你说的/楼上提到的」，更不能用「这个我也很赞」「说得对」之类去附和它——那等于把搜到的东西栽给了帖子。\n' +
      '3. 确实想引用搜索资料时，必须让人看出那是你自己查到的，例如「我查了下，这个方案的应用场景还包括…」，而不是让人以为帖子里写了。\n' +
